...
from .bpic_signals import FEATURE_KEYS as BPIC_FEATURES
from .bpic_signals import build_panel as build_bpic
...
def run_real(seeds: tuple[int, ...] = (11, 23, 37, 41, 53)) -> pd.DataFrame:
    """The same sweep on BPIC 2015, which is the arm a reviewer will ask for.

    The synthetic sweep can vary the regime and the seed and hold everything
    else fixed, which is why it exists — but its panels are generated by the
    same process that defines the signals, and a reader is entitled to ask
    whether the trade-off survives contact with a real organisation.

    Grouped by case worker so no unit's quarters straddle a fold, and the floor
    is applied only to the tuner fitted on each training split: the held-out
    quarters never influence the weights whose cost is being measured.
    """
    spec = load_spec()
    panel = build_bpic()
    X = panel[BPIC_FEATURES].astype(float).reset_index(drop=True)
    y = panel["failed"].to_numpy()
    groups = (panel["municipality"].astype(str) + "/" + panel["owner"].astype(str)).to_numpy()

    base = InterpretableScorer(spec, "SECTOR")
    uniform = 1.0 / len(base.feature_keys)
    ...
    for seed in seeds:
        ...
        for fold, (train_idx, test_idx) in enumerate(splitter.split(X, y, groups)):
            ...
            untuned = roc_auc_score(y_test, base.predict_proba(X_test)[:, 1])

            for fraction in FLOOR_FRACTIONS:
                floor = fraction * uniform
                result = tune_weights(
                    spec, "SECTOR", X_train, y_train, seed=seed, min_weight=floor
                )
                scorer = InterpretableScorer(spec, "SECTOR", weights=result.weights)
                rows.append(
                    {
                        "seed": seed,
                        "fold": fold,
                        "floor_fraction": fraction,
                        "min_weight": floor,
                        "test_auc": roc_auc_score(y_test, scorer.predict_proba(X_test)[:, 1]),
                        "untuned_auc": untuned,
                        "hhi": herfindahl(result.weights),
                        "largest_share": largest_share(result.weights),
                        "converged": result.converged,
...
def report_real(frame: pd.DataFrame) -> None:
    print("--- the same sweep on BPIC 2015 (real data) ---")
    print(f"    {frame.seed.nunique()} repeats x {frame.fold.nunique()} folds, "
          f"{len(frame)} fits\n")
    print(f"  {'floor':>8} {'test AUC':>9} {'sd':>7} {'HHI':>7} {'largest factor':>15}")
    for fraction in FLOOR_FRACTIONS:
        g = frame[frame.floor_fraction == fraction]
        label = "free" if fraction == 0 else f"{fraction:.0%}"
        print(f"  {label:>8} {g.test_auc.mean():>9.4f} {g.test_auc.std():>7.4f} "
              f"{g.hhi.mean():>7.3f} {g.largest_share.mean():>14.1%}")

    free = frame[frame.floor_fraction == 0]
    tight = frame[frame.floor_fraction == FLOOR_FRACTIONS[-1]]
    print(f"\n    untuned (shipped weights) : {frame.untuned_auc.mean():.4f}")
    print(f"    free -> tightest floor    : {tight.test_auc.mean() - free.test_auc.mean():+.4f} AUC")
    print(f"    concentration falls       : {free.hhi.mean():.3f} -> {tight.hhi.mean():.3f}")
    print(f"    largest factor falls      : {free.largest_share.mean():.1%} "
          f"-> {tight.largest_share.mean():.1%}")

    swing = frame.groupby("floor_fraction").test_auc.mean()
    print(f"\n  Largest mean AUC difference across the range: {swing.max() - swing.min():.4f}")

    # Direction matters as much as magnitude here. A swing of 0.006 that runs
    # *towards* the constrained end is not a price, and calling it one would
    # misreport the finding in the direction least favourable to the argument
    # being made — which is still misreporting.
    cost = free.test_auc.mean() - tight.test_auc.mean()
    collapsed = free.largest_share.mean() > 0.5
    if cost <= 0:
        print(f"  The floor costs NOTHING on real data: the tightest setting scores "
              f"{-cost:+.4f} against free optimisation.")
    elif cost < 0.005:
        print("  On real data the constraint is effectively free.")
    else:
        print(f"  On real data the constraint costs {cost:.4f} AUC. Report the curve.")

    if not collapsed:
        print(f"  Note: unconstrained tuning did NOT collapse here — the largest "
              f"factor reaches only {free.largest_share.mean():.1%}. The pathology the "
              f"floor exists to prevent did not occur on this panel, so this arm "
              f"measures the floor's cost when it is NOT needed.")
